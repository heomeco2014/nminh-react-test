...
>(({active, payload, className, indicator = 'dot', hideLabel = false, hideIndicator = false, label, labelFormatter, labelClassName, formatter, color, nameKey, labelKey}, ref) => {
	...
	return (
		<div ref={ref} className={cn('grid min-w-[8rem] items-start gap-1.5 rounded-[4px] border border-neutral-200 border-neutral-200/50 bg-white px-2.5 py-1.5 text-xs shadow-xl dark:border-neutral-800 dark:border-neutral-800/50 dark:bg-neutral-950', className)}>
			...
			<div className='grid gap-1.5'>
				{payload.map((item, index) => {
					...
					return (
						<div key={item.dataKey} className={cn('flex w-full flex-wrap items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5 [&>svg]:text-neutral-500 dark:[&>svg]:text-neutral-400', indicator === 'dot' && 'items-center')}>
